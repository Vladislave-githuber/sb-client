  ...
  const renderReport = () => {
    ...
    switch (activeTab) {
        ...
        return (
          <div>
            ...
            <table className="report-table"><thead><tr><th>ID операции</th><th>Тип</th><th>Прибыль (BYN)</th><th>Описание</th><th>Дата</th></tr></thead><tbody>
              {data.details?.map((d: any) => (<tr key={d.transactionId}><td>{d.transactionId?.slice(0,8)}</td><td>{d.type}</td><td className={safeNumber(d.profit)>=0?'profit':'loss'}>{safeNumber(d.profit).toFixed(2)}</td><td>{d.description}</td><td>{new Date(d.createdAt).toLocaleString()}</td></tr>))}
            </tbody></table>
          </div>
        );
      case 'margin':
      ...
      case 'consolidated':
        ...
        return (
          <table className="report-table"><thead><tr><th>Валюта</th><th>BUY</th><th>SELL</th><th>CONVERT</th><th>Всего</th></tr></thead><tbody>
